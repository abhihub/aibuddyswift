...
class ChatService: ObservableObject {
    ...
    func sendMessage(_ text: String, screenshotPath: String? = nil) async {
        ...
        // Simple mock response for demo - replace with actual OpenAI API call
        try? await Task.sleep(nanoseconds: 1_000_000_000) // 1 second delay
        
        let response = screenshotPath != nil 
            ? "I can see your screenshot! You said: \(text). The screenshot was saved to: \(screenshotPath!)" 
            : "You said: \(text)"
...
// MARK: - App Launcher (No @main attribute)
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        window.title = "buddyChatGPT - ChatGPT with Screenshots"
    }
...
// Main entry point - no @main attribute conflicts
let app = NSApplication.shared
let delegate = AppDelegate()
app.delegate = delegate
print("🚀 Starting buddyChatGPT...")
print("💡 Type a message and it will automatically take a screenshot!")
app.run()